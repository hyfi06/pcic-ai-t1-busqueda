...
def crossing(parent_1: U, parent_2: U) -> Tuple[U, U]:
    ...
    return (
        parent_1.__class__(chromosome=child_1),
        parent_1.__class__(chromosome=child_2)
    )
...
def genetic(
    ...
    while timer():
        ...
        sys.stdout.write(
            f"\rGeneration: {generation} Fit: {fitness(max_individual)} Conflicts: {(1.0/fitness(max_individual))-1}")
        sys.stdout.flush()
